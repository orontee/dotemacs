...
#+begin_src emacs-lisp :tangle init.el
  (with-eval-after-load "follow"
    ...
    (keymap-set follow-mode-submap "f" 'follow-mode))
...
When available, use =ugrep= to search in files; It's damned fast!

#+begin_src emacs-lisp :tangle init.el
  (when (executable-find "ugrep")
    (setopt grep-find-command
  	    '("ugrep -n -H --color=always --no-heading -e '' $(git rev-parse --show-toplevel || pwd)"
  	      . 27))
    (setopt grep-find-template "ugrep -n -H --color=always --no-heading -e <R> <D>")
    (setopt grep-find-use-xargs 'gnu-sort)
    (setopt grep-template "ugrep -n -H --color=always --no-heading -e <R> <F>")
    (setopt grep-use-null-device nil)
    (setopt grep-highlight-matches t)

    (with-eval-after-load "xref"
      (when (assoc 'ugrep xref-search-program-alist)
