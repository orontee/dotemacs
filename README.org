...
system (whatever it is: Mostly Debian and, at work, Microsoft
Windows).  But suprisingly I feel like it has adapted to my needs
rather than I had to limit myself to its features.
...
#+begin_src sh
  ...
  $ ./autogen.sh; ./configure --prefix "/usr/local" \
  			    --with-dbus --with-gif --with-gnutls --with-jpeg \
  			    --with-libsystemd --with-mailutils --with-modules \
  			    --with-native-compilation --with-pgtk --with-png --with-rsvg \
  			    --with-tiff --with-tree-sitter --with-xft --with-xinput2 \
  			    --with-imagemagick --with-xpm --without-xaw3d \
...
The standard way to extend Emacs features is to use the built-in
package manager. Not all libraries are available in the form of
packages, so when a library must be installed by hand, the directory
=~/.config/emacs/lisp= is used. The load path is extended accordingly.

#+begin_src emacs-lisp :tangle init.el
  (add-to-list 'load-path (expand-file-name "~/.config/emacs/lisp/"))
#+end_src

Packages configuration is done with =use-package=.
...
is dedicated to bookmark names. Completion of bookmark name uses
substring style. Finally ease bookmarks switching.
...
#+begin_src emacs-lisp :tangle init.el
  (use-package bookmark
    ...
    (defun bookmark-project-bookmarks ()
      "Return a bookmarks file name build from project name."
      (let ((project (project-current)))
        (if project
            (expand-file-name
             (concat "bookmarks-" (project-name project) ".eld")
             user-emacs-directory)
  	bookmark-default-file)))

    (defcustom bookmark-switch-bookmarks-default-file nil
      "Determines the default bookmarks file to load with
       `bookmark-switch-bookmarks'.

       When nil, `bookmark-default-file' is used.

       When equal to a callable, it is expected to return the bookmarks
       file to use by `bookmark-switch-bookmarks' as default bookmarks
       file to use."
      :version "32.0"
      :group 'bookmark
      :type '(choice (const :tag "Default bookmarks file" nil)
  		   (function :tag "Custom function")))

    (setopt bookmark-switch-bookmarks-default-file
  	  'bookmark-project-bookmarks)

    (defun bookmark-switch-bookmarks-candidate ()
      "Return bookmarks file to use when switching bookmarks.

      When `bookmark-switch-bookmarks-function' is callable, then this
      function is called without argument and its value is
      returned. Otherwise `bookmark-defaul-file' is returned."
      (cond
       ((functionp bookmark-switch-bookmarks-default-file)
        (funcall bookmark-switch-bookmarks-default-file))
       (t bookmark-default-file)))

    (defun bookmark-switch-bookmarks (file)
      "Switch to bookmarks from FILE.

       The \Bookmark List\ is updated to display only bookmarks from
       FILE. Subsequent call to `bookmark-bmenu-save' update FILE. Note
       that `bookmark-default-file' is kept unchanged."
      (interactive (list
                    (read-file-name "Bookmarks file: "
                                    user-emacs-directory
  				  (bookmark-switch-bookmarks-candidate)
                                    (lambda (file)
    ...
    :bind
    (:map ctl-x-r-map ("L" . bookmark-switch-bookmarks))
    (:map bookmark-bmenu-mode-map ("L" . bookmark-switch-bookmarks)))
#+end_src
...
#+begin_src emacs-lisp :tangle init.el
  ...
  (use-package remember
      ...
      (let ((desc (remember-buffer-desc))
  	  (text (string-trim
  		 (mapconcat
  		  ...
  		  (cdr
  		   (split-string
  		    text "\n" nil split-string-default-separators))
  		  "\n"))))
        (with-temp-buffer
    ...
    (setopt remember-data-files-regex "\\.md\\'")
  ...
  The note is moved to `remembre-markdown-archive-file'."
    ...
    :config
    (setq remember-initial-major-mode 'markdown-mode
...
=conan= configuration files are Conf files.
    ...
    (let ((fullscreen (frame-parameter (window-frame) 'fullscreen))
      ...
      (if fullscreen
          (setq display-buffer-alist
  		   ...
  		   "\\*eldoc.*\\*\\|"
  		   (regexp-opt
  		    '("*xref*" "*grep*" "*Occur*" "*Help*" "*Apropos*" "*Messages*"
...
A key binding is dedicated to select a given dedicated window.

#+begin_src emacs-lisp :tangle init.el
  (defun other-dedicated-window (window)
    """Select another dedicated window. """
    (interactive
     (list
      (let ((frame-windows (window-list-1 nil 'skip-minibuf (window-frame)))
  	  (side-windows-buffers))
        (dolist (w frame-windows)
  	(when (window-dedicated-p w)
  	  (let ((name (buffer-name (window-buffer w))))
  	    (push `(,name . ,w) side-windows-buffers))))
        (cond
         ((eq (length side-windows-buffers) 0)
  	(error "No side window"))
         ((eq (length side-windows-buffers) 1)
  	(cdr (car side-windows-buffers)))
         (t (cdr (assoc
  		(completing-read "Buffer: " side-windows-buffers)
  		side-windows-buffers)))))))
    (select-window window))

  (keymap-global-set "C-x M-o" 'other-dedicated-window)

#+end_src

Help follow cursor with pulses. A key binding is defined to pulse on
demand.
...
Define a key binding for =follow-delete-other-windows-and-split= and
when follow mode is active use same key binding to disable follow
mode.
...
#+begin_src emacs-lisp :tangle init.el
  (with-eval-after-load "follow"
    ...
    (setopt follow-mode-prefix-key "C-x f")
    (keymap-set follow-mode-submap "f"
    	      'follow-switch-to-current-buffer-all)
    (keymap-set follow-mode-submap "q" 'follow-mode-quit))

  (define-prefix-command 'global-follow-mode-map)
  (keymap-global-set "C-x f" 'global-follow-mode-map)
  (keymap-global-set "C-x f f" 'follow-delete-other-windows-and-split)
#+end_src
...
When reading file or buffer names the case will be ignored. For file
names, backup files are hidden from completion candidates.
...
#+begin_src emacs-lisp :tangle init.el
  ...
  Omit the .. directory from FILES."
    ...
    (let ((backup-p (lambda (file) (string-suffix-p "~" file))))
      (seq-remove backup-p files)))
...
Make manpages the current buffer in the other window.
...
Two custom themes based on the excellent =modus-themes= are
provided. They are written for maximal accessible readability.
...
#+begin_src emacs-lisp :tangle init.el
  ...
  (load-theme 'matthem-dark :no-confirm)
#+end_src
...
Use =guess-language-mode= to select spell check language automatically
and configure it to be less intrusive in mode-line.
  ...
  (use-package guess-language
    ...
    (setq guess-language-languages '(en fr)
  	...
  	'((en "en" "English" "📚️en" "English")
  	  (fr "francais" "French" "📚️️fr" "French")))
    :hook text-mode)
#+end_src
...
Eglot is used to start LSP servers. A dedicated prefix map is created
and populated.
...
#+begin_src emacs-lisp :tangle init.el
  (use-package eglot
    ...
    (eglot-managed-mode . (lambda ()
  			  (keymap-local-set "C-c i" 'eglot-prefix-map)
  			  (keymap-set eglot-prefix-map "i" 'eglot-find-implementation)
  			  (keymap-set eglot-prefix-map "e" 'eglot)
  			  (keymap-set eglot-prefix-map "k" 'eglot-shutdown-all)
  			  (keymap-set eglot-prefix-map "r" 'eglot-rename)
  			  (keymap-set eglot-prefix-map "x" 'eglot-reconnect)
  			  (keymap-set eglot-prefix-map "a" 'eglot-code-actions)
  			  (keymap-set eglot-prefix-map "m" 'eglot-menu)
  			  (keymap-set eglot-prefix-map "f" 'eglot-format-buffer)
  			  (keymap-set eglot-prefix-map "h" 'eglot-inlay-hints-mode))))
#+end_src
...
Flymake is used to check code on the fly. A dedicated prefix map is
...
#+begin_src emacs-lisp :tangle init.el
  (use-package flymake
    ...
    :hook
    (flymake-mode . (lambda ()
  		    (keymap-local-set "C-c F" 'flymake-prefix-map)
                      (keymap-set flymake-prefix-map "d" 'flymake-show-buffer-diagnostics)
                      (keymap-set flymake-prefix-map "n" 'flymake-goto-next-error)
                      (keymap-set flymake-prefix-map "p" 'flymake-goto-prev-error))))
#+end_src

** Code navigation

XRef is used to navigate between references. In the results buffer
current line is highlighted and some files are always outlined. 
...
Finally a breadcrum is added to buffers visiting code files.
...
string checked for right spelling.
...
Currently transitionning from core "VC" to Magit.

#+begin_src emacs-lisp :tangle init.el
  (use-package magit
    :config
    (setq magit-define-global-key-bindings 'recommended)
    (add-hook 'magit-log-mode-hook (lambda () (text-scale-set -1)))
    (add-hook 'magit-stashes-mode-hook (lambda () (text-scale-set -1)))
    (add-hook 'magit-status-mode-hook (lambda () (text-scale-set -1)))
    (add-hook 'magit-refs-mode-hook (lambda () (text-scale-set -1)))
    :bind
    (:map project-prefix-map
  	("v" .  magit-status)))
#+end_src

Follows previous "VC" configuration.
...
Easy generation of URLs for Git projects hosted on common forges.
...
#+begin_src emacs-lisp :tangle init.el
  (use-package coterm
    :ensure nil
    :vc (:url "https://repo.or.cz/emacs-coterm.git"
          :rev newest)
    :config
    (coterm-mode 1))
#+end_src
...
Colorize compilation output, scale text height and enable URL parsing.
...
#+begin_src emacs-lisp :tangle init.el
  (use-package markdown-mode
    :init
    (define-skeleton markdown-create-document
      "Create a Markdown structured document"
      "Title: " "---\ntitle: " str ?\n "tags: []"_ ?\n"---\n\n# " str ?\n)
    :custom
...
And improve readability in both modes.
...
#+begin_src emacs-lisp :tangle init.el
  (use-package olivetti
    ...
    :vc (:url "https://github.com/rnkn/olivetti.git"
  	    :rev newest)
    :custom
    (olivetti-body-width 90)      (olivetti-style 'fancy)
    (olivetti-minimum-body-width 80)
    :config
    (keymap-unset olivetti-mode-map "<left-margin> <mouse-1>")
    (keymap-unset olivetti-mode-map "<right-margin> <mouse-1>")
    (keymap-unset olivetti-mode-map "<left-fringe> <mouse-1>")
    (keymap-unset olivetti-mode-map "<right-fringe> <mouse-1>")
    (keymap-unset olivetti-mode-map "<left-margin> <mouse-1>")
    (keymap-unset olivetti-mode-map "<right-margin> <mouse-1>")
    (keymap-unset olivetti-mode-map "<left-fringe> <mouse-1>")
    (keymap-unset olivetti-mode-map "<right-fringe> <mouse-1>")
    :hook (Info-mode Man-mode markdown-mode markdown-ts-mode))
#+end_src
