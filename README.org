...
#+begin_src emacs-lisp :tangle init.el
(defun bookmark-bookmark-buffer ()
  (save-excursion
...
(when (featurep 'tab-bar)
  (setq tab-bar-close-button-show t
        ...
        tab-bar-new-tab-choice 'bookmark-bookmark-buffer
...
`project-find-file', `project-find-regexp', `project-search' and
`project-query-replace-regexp' commands.

What's more the `compile' command is adviced to always run in the
first root of the current project.
...
(when (locate-library "project")
  ...
  (define-key project-prefix-map "c" 'compile)
  ...
  (defun my-compile-advice (orig &rest args)
    (let ((default-directory (or (cdr (project-current)) default-directory)))
	(apply orig args)))
  (advice-add 'compile :around #'my-compile-advice))
#+end_src
...
#+begin_src emacs-lisp :tangle init.el
  (add-hook 'yaml-mode-hook
	    (lambda ()
	      (seq electric-quote-replace-double nil)))
#+end_src
...
(setq org-hide-leading-stars t
      org-log-done 'time
      org-directory (expand-file-name "~/.config/emacs/org")
