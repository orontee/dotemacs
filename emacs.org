...
#+options: H:2 toc:1 todo:t email:t ^:nil
...
I found that tool more or less randomly sitting in front of one of the
X Window terminals of my university. Today, whatever I have to do with
a computer, it ends in an Emacs buffer... In my current use, Emacs has
become a kind of interface over the computer's operating system
(whatever it is: Arch Linux, Ubuntu, Windows XP and Vista). But
suprisingly I feel like it has adapt to my needs rather than I have
limited myself to its features.

As a intensive Emacs user I had various needs related to Emacs
...
tools; etc. Thus I have written an Org file whose code blocks could be
converted to the source file I use to customize emacs to my needs and
that can be converted to the documentation you are reading.
...
All settings written using the customize library goes to a dedicated
...
bars, scroll bars and tool bars must be disabled. Whats more disabling
the eyecandy may improve responsiveness when Emacs run on a remote
computer forwarding X11. Note that the content of the menu bar is
still available by pressing =f10= or the contextual menu bind to
=C-mouse-1=.
...
(dolist (hook '(gdb-breakpoints-mode-hook gdb-inferior-io-mode-hook
                ...
                speedbar-mode compilation-mode-hook vc-dir-mode-hook
...
reordered to privilege C++ usual extensions rather than C ones (see
