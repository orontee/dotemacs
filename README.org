...
Trying LSP servers through Eglot, thus default is to keep Semantic disabled.
...
(eval-after-load "xref"
  (progn
    (defun xref-forward-header ()
	(interactive)
	(xref--search-property 'xref-group))

    (defun xref-backward-header ()
	(interactive)
	(xref--search-property 'xref-group t))

    (define-key xref--xref-buffer-mode-map (kbd "C-M-p") #'xref-backward-header)
    (define-key xref--xref-buffer-mode-map (kbd "C-M-n") #'xref-forward-header)))
#+end_src
